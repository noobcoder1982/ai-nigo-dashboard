...
# Set standard paths to allow imports from root
...
def call_llm_MASTER(description, tone="Professional", temperature=0.1):
    ...
    try:
        r = requests.post(NVIDIA_API_URL, headers=headers, json=payload, timeout=30)
        r.raise_for_status()
        c = r.json()['choices'][0]['message']['content'].replace("```json", "").replace("```", "").strip()
        return json.loads(c)
    except Exception as e: 
        print(f"DEBUG LLM ERROR: {e}")
        return {"category": "General", "urgency": "Medium", "people_count": 5, "thought_process": "Fallback mode. Error: " + str(e), "understood_reasoning": "Standard parsing applied."}
...
if 'res' in st.session_state:
    ...
    with c_brief:
        ...
        # --- NEW: AI THINKING SECTION ---
        if st.button("🧠 INSPECT AI THINKING", use_container_width=False):
            st.session_state.show_thinking = not st.session_state.get('show_thinking', False)
        
        if st.session_state.get('show_thinking', False):
            st.text_area("AI Internal Understanding & Reasoning Trace", 
                         value=st.session_state.res['thoughts'], 
                         height=200,
                         help="This shows what the AI interpreted from your natural language input.")
    with c_dispatch:
